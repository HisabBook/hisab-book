...
import { getVazirmatnBase64 } from './fontBase64Loader';
...
const createTextFormatter = (language) => {
  ...
  const shaper = reshaperModule?.PersianShaper ?? reshaperModule?.default?.PersianShaper;
  return (value) => {
    const text = String(value ?? '');
    if (!shaper?.convertArabic) return text;
...
export const generateInvoicePDF = async ({
  ...
  try {
    ...
    if (rtl) {
      const vazirmatnBase64 = getVazirmatnBase64();
      if (vazirmatnBase64) {
